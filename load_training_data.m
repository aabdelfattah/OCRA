function [eval_data, data] = load_training_data(setdir,NUM_LETTERS,TRAINING_SAMPLES,EVAL_SAMPLES)
    ...
    for i = 1 : NUM_LETTERS
        ...
        for j = 1 : EVAL_SAMPLES
            eval(['pic = load (''' eval_files(j).name ''');']);  
            [l w] = size(pic);
            xprev = pic(1,1);
            yprev = pic(1,2);
            eval_o = ones(1,l-1);
            for k = 2 : l % load features and quantize it
                x = pic(k,1);
                y = pic(k,2);
                eval_o(k-1) = quantize(atan2d(y-yprev,x-xprev));
                xprev = x;
                yprev = y;

            end
            eval_data{i,j} = eval_o; % for each letter i load all samples j , j +1 .. 
        end
        cd ..
